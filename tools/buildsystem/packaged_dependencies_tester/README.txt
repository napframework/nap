...
Usage:
    ...
    5. python3 packaged_dependencies_tester.py NAP-0.2.3-Win64
...
Future potential changes:
    ...
    - Adding options for report output location and working directory for packaged apps (if tying into
      automated testing)
    ...
    - Make a set of test apps which include a minimum set of NAP modules, identifying cases where some
      modules may unintentionally only work when combined with others. This would require a separate platform
      build that includes these apps though, which maybe could involve the script itself wrapping a
      packaging process.
