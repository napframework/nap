...
RTTI_PROPERTY("Controller",				&nap::FacadeLightComponent::mArtnetController,		nap::rtti::EPropertyMetaData::Required)
	...
	RTTI_PROPERTY("IntensityComponent", &nap::FacadeLightComponent::mIntensityComponent,	nap::rtti::EPropertyMetaData::Required)
RTTI_END_CLASS
...
namespace nap
{
	...
	bool FacadeLightComponentInstance::init(utility::ErrorState& errorState)
	{
		...
		mOpeningData = std::vector<uint8_t>(count, math::max<uint8_t>());
		mClosingData = std::vector<uint8_t>(count, 0);
		return true;
	}
	...
	{
		// Don't update if send is explicitly turned off
		if (!mSend) { return; }
		
		// Turn lights off when we take into account shopping hours and the stores are closed
		if (mIntensityComponent->mUseOpeningHours && !mIntensityComponent->isOpen())
		{
			mController->send(mClosingData, mMinChannel);
		}
		else
		{
			mController->send(mOpeningData, mMinChannel);
		}
	}
}