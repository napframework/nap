...
RTTI_BEGIN_STRUCT(nap::emography::Snapshot)
	RTTI_VALUE_CONSTRUCTOR(nap::emography::EState, float)
...
namespace nap
{
	namespace emography
	{
		...
		Snapshot::Snapshot(EState state, float intensity) : 
			mState(state), mIntensity(intensity), mTimeStamp(utility::getCurrentTime())
		{ }


		Snapshot::Snapshot(EState state, float intensity, utility::SystemTimeStamp stamp) :
