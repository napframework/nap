...
class OperatorItem(InputOutputNodeItem):
    def __init__(self, name, op: Operator):
        super(OperatorItem, self).__init__(name)
        self.operator = op
...
class PatchScene(GraphScene):
    ...
    def createOperator(self, name, opType, pos):
        op = opType(self.patch)
        op.x(pos.x())
        op.y(pos.y())
        item = OperatorItem(name, op)
        item.setPos(QPointF(op.x(), op.y()))
        self.patch.add(name, op)
        self.addNode(item)
...
class PatchView(GraphView):
    ...
    def showRegularMenu(self, pos):
        menu = QMenu()
        for action in createOperatorActions(menu, self.scene(), self.mapToScene(pos)):
