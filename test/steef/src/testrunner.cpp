...
namespace nap {
	...
	: mVinylLabelImg(nullptr)
	, mVinylCoverImg(nullptr)
	, mRenderService(nullptr)
	, mResourceManagerService(nullptr)
	, mSceneService(nullptr)
	, mInputService(nullptr)
	, mModelEntity(nullptr)
	 ...
	 */
	bool TestRunner::init(nap::Core& core)
	{
		core.initialize();
		...
		mRenderWindow->onWindowEvent.connect(std::bind(&TestRunner::handleWindowEvent, this, std::placeholders::_1));
	...
	void TestRunner::onUpdate()
	{
		// Update input for first window
		std::vector<nap::EntityInstance*> entities;
		entities.push_back(mCameraEntity.get());
		mInputService->processEvents(*mRenderWindow, mInputRouter, entities);
	 ...
	 */
	void TestRunner::handleWindowEvent(const nap::WindowEvent& windowEvent)
	{
	...
	void TestRunner::setWindowFullscreen(std::string windowIdentifier, bool fullscreen) {
	 ...
	 */
	void TestRunner::updateBackgroundImage()
	{
		// Get size
