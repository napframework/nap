...
namespace nap
{
        ...
        performEditAction([this, trackID, time, &return_ptr]() mutable
        {
            ...
            for (auto& track : sequence.mTracks)
            {
                if (track->mID==trackID)
                {
                    ...
                    {
                        if (segment->mStartTime<time && segment->mStartTime+segment->mDuration>time)
                        {
                            // segment found
                            ...
                            new_segment->setStartValue(segment_curve_2.getValue((segment->mStartTime+segment->mDuration-time)/segment->mDuration));
    ...
    void SequenceControllerCurve::insertCurvePoint(SequenceTrackSegment& segment, float pos, int curveIndex)
    {
        ...
        for (int i = 0; i<curve_segment.mCurves[curveIndex]->mPoints.size()-1; i++)
        {
            ...
            if (curve_segment.mCurves[curveIndex]->mPoints[i].mPos.mTime<=pos && curve_segment.mCurves[curveIndex]->mPoints[i+1].mPos.mTime>pos)
            {
                // create point
