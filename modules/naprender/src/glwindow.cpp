...
namespace nap
{
	...
	*/
	...
	static bool createFramebuffers(VkDevice device, std::vector<VkFramebuffer>& framebuffers, std::vector<VkImageView>& swapChainImageViews, VkRenderPass renderPass, VkExtent2D extent)
	{
	...
	bool GLWindow::init(const RenderWindowSettings& settings, VkInstance vulkanInstance, VkPhysicalDevice physicalDevice, VkDevice device, VkCommandPool commandPool, unsigned int graphicsQueueIndex, nap::utility::ErrorState& errorState)
	{
		mDevice = device;
		...
		if (!createRenderPass(device, swapchainFormat, mRenderPass))
			return false;

		if (!createImageViews(device, mSwapChainImageViews, chain_images, swapchainFormat))
			return false;

		if (!createFramebuffers(device, mSwapChainFramebuffers, mSwapChainImageViews, mRenderPass, swapchainExtent))
			return false;
