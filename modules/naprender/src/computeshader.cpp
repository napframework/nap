...
RTTI_BEGIN_CLASS_NO_DEFAULT_CONSTRUCTOR(nap::ComputeShaderFromFile)
	...
	RTTI_PROPERTY_FILELINK("ComputeShader", &nap::ComputeShaderFromFile::mComputePath, nap::rtti::EPropertyMetaData::Required, nap::rtti::EPropertyFileType::VertShader)
RTTI_END_CLASS
...
static bool compileProgram(VkDevice device, uint32_t vulkanVersion, const char* compSource, int compSize, const std::string& shaderName, std::vector<nap::uint32>& computeSPIRV, nap::utility::ErrorState& errorState)
{
...
namespace nap
{
	...
	ComputeShader::ComputeShader(Core& core) : mRenderService(core.getService<RenderService>())
	{ }
	...
	ComputeShader::~ComputeShader()
	{
		...
		mRenderService->queueVulkanObjectDestructor([descriptorSetLayout = mDescriptorSetLayout, computeModule = mComputeModule](RenderService& renderService)
		{
			if (descriptorSetLayout != nullptr)
				vkDestroyDescriptorSetLayout(renderService.getDevice(), descriptorSetLayout, nullptr);

			if (computeModule != nullptr)
				vkDestroyShaderModule(renderService.getDevice(), computeModule, nullptr);
		});
	}
	...
	bool ComputeShader::load(const std::string& displayName, const char* compShader, int compSize, utility::ErrorState& errorState)
	{
		...
		if (!compileProgram(device, vulkan_version, compShader, compSize, mDisplayName, comp_shader_spirv, errorState))
			return false;

		// Create compute shader module
		mComputeModule = createShaderModule(comp_shader_spirv, device);
		if (!errorState.check(mComputeModule != nullptr, "Unable to load fragment shader module"))
			return false;
		...
		if (!parseUniforms(comp_shader_compiler, VK_SHADER_STAGE_VERTEX_BIT, mUBODeclarations, mSamplerDeclarations, errorState))
			return false;

		for (const spirv_cross::Resource& stage_input : comp_shader_compiler.get_shader_resources().stage_inputs)
		{
			spirv_cross::SPIRType input_type = comp_shader_compiler.get_type(stage_input.type_id);
			...
			uint32_t location = comp_shader_compiler.get_decoration(stage_input.id, spv::DecorationLocation);
	...
	bool ComputeShader::initLayout(VkDevice device, nap::utility::ErrorState& errorState)
	{
	...
	bool ComputeShaderFromFile::init(utility::ErrorState& errorState)
	{
		...
		std::vector<char> comp_source;
		if (!errorState.check(tryReadFile(mComputePath, comp_source), "Unable to read shader file %s", mComputePath.c_str()))
			return false;
