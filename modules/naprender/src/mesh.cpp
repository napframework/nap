...
namespace nap
{
	...
	bool MeshInstance::initGPUData(utility::ErrorState& errorState)
	{
		...
		{
			IGPUBufferNumeric* buffer = nullptr;
	...
	bool MeshInstance::update(utility::ErrorState& errorState)
	{
		...
		for (auto& mesh_attribute : mProperties.mAttributes)
		{
			IGPUBufferNumeric& vertex_attr_buffer = mGPUMesh->getVertexBuffer(mesh_attribute->mAttributeID);
		...
		{
			IGPUBufferNumeric& index_buffer = mGPUMesh->getOrCreateIndexBuffer(shapeIndex);
	...
	{
		IGPUBufferNumeric& vertex_attr_buffer = mGPUMesh->getVertexBuffer(attribute.mAttributeID);
