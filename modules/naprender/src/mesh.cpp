...
namespace nap
{
	...
	bool MeshInstance::update(utility::ErrorState& errorState)
	{
		...
		for (auto& mesh_attribute : mProperties.mAttributes)
		{
			if (!errorState.check(mesh_attribute->getSize() == mProperties.mNumVertices,
					"Vertex attribute %s has a different amount of elements (%d) than the mesh (%d)", mesh_attribute->mAttributeID.c_str(), mesh_attribute->getSize(), mProperties.mNumVertices))
			{
				return false;
			}
		}
