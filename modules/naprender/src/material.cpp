	...
	*/
	...
	bool MaterialInstance::init(MaterialInstanceResource& resource, utility::ErrorState& errorState)
	{
		...
		for (ObjectPtr<Uniform>& uniform : resource.mUniforms)
		{
			...
			{
				nap::Logger::warn("Unable to find uniform %s in shader %s for material %s", uniform->mName.c_str(), resource.mMaterial->getShader()->mID.c_str(), resource.mMaterial->mID.c_str());
				continue;
			}
	...
	bool Material::init(utility::ErrorState& errorState)
	{
		...
		for (ObjectPtr<Uniform>& uniform : mUniforms)
		{
			...
			if (declaration == uniform_declarations.end())
			{
				nap::Logger::warn("Unable to find uniform %s in shader %s", uniform->mName.c_str(), mShader->mID.c_str());
				continue;
			}
		}
	...
	std::vector<Material::VertexAttributeBinding>& Material::getDefaultVertexAttributeBindings()
	{
		...
		if (bindings.empty())
		{
			bindings.push_back({ MeshInstance::VertexAttributeIDs::GetPositionName(), opengl::Shader::VertexAttributeIDs::GetPositionVertexAttr() });
			bindings.push_back({ MeshInstance::VertexAttributeIDs::getNormalName(), opengl::Shader::VertexAttributeIDs::GetNormalVertexAttr() });

			const int numChannels = 4;
			for (int channel = 0; channel != numChannels; ++channel)
			{
				bindings.push_back({ MeshInstance::VertexAttributeIDs::GetColorName(channel), opengl::Shader::VertexAttributeIDs::GetColorVertexAttr(channel) });
				bindings.push_back({ MeshInstance::VertexAttributeIDs::GetUVName(channel), opengl::Shader::VertexAttributeIDs::GetUVVertexAttr(channel) });
			}
		}
		return bindings;
	}
	...
	const Material::VertexAttributeBinding* Material::findVertexAttributeBinding(const opengl::GPUMesh::VertexAttributeID& shaderAttributeID) const
	{
