...
namespace nap
{
	...
	bool SlideShowComponentInstance::init(utility::ErrorState& errorState)
	{
	...
	void SlideShowComponentInstance::assignTexture(nap::EntityInstance& entity, int imageIndex)
	{
		...
		material_instance.getOrCreateUniform<nap::Sampler2D>("mTexture").setTexture(*resource->mImages[imageIndex]);
	}
