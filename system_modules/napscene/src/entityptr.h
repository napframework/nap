...
namespace nap
{
	...
	public:
		...
		EntityPtr(Entity* entity) : mResource(entity)
		{ }
		 ...
		 */
		Entity* toObject() const												{ return mResource.get(); }
		...
		const Entity& operator*() const											{ assert(mResource != nullptr); return *mResource; }

		Entity& operator*()														{ assert(mResource != nullptr); return *mResource; }

		const Entity* operator->() const										{ assert(mResource != nullptr);	return mResource.get(); }

		Entity* operator->()													{ assert(mResource != nullptr); return mResource.get(); }
	...
	*/
	...
	public:
		...
		const EntityInstance& operator*() const									{ assert(mInstance != nullptr); return *mInstance; }

		EntityInstance& operator*()												{ assert(mInstance != nullptr); return *mInstance; }

		EntityInstance* operator->() const										{ assert(mInstance != nullptr);	return mInstance; }

		EntityInstance* operator->()											{ assert(mInstance != nullptr);	return mInstance; }

		bool operator==(const EntityInstancePtr& other) const					{ return mInstance == other.mInstance; }
		...
		EntityInstance* get() const												{ return mInstance; }

		EntityInstance* get()													{ return mInstance; }
...
namespace rttr
{
	...
	struct wrapper_mapper<nap::EntityPtr>
	{
		...
		inline static type create(const wrapped_type& value)	{ return nap::EntityPtr(value); }
	};
}
