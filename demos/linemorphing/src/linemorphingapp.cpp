...
RTTI_BEGIN_CLASS_NO_DEFAULT_CONSTRUCTOR(nap::LineMorphingApp)
...
namespace nap 
{
	 ...
	 */
	bool LineMorphingApp::init(utility::ErrorState& error)
	{
		// Retrieve services
		...
		mLineRotationParam = mResourceManager->findObject<ParameterFloat>("line_rotation");
	 ...
	 */
	void LineMorphingApp::update(double deltaTime)
	{
		...
		mLineEntity->getComponent<RotateComponentInstance>().setSpeed(mLineRotationParam->mValue);
	}
	 ...
	 */
	void LineMorphingApp::windowMessageReceived(WindowEventPtr windowEvent)
	{
	 ...
	 */
	void LineMorphingApp::inputMessageReceived(InputEventPtr inputEvent)
	{
