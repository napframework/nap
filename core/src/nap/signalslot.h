...
namespace nap 
{
    ...
    **/
    ...
    public:
        ...
        void connect(U* parent, F memberFunction) { connect(std::bind(memberFunction, parent, std::placeholders::_1)); }
    ...
    **/
	...
	{
    public:
        //@name The function pointer needs to be redeclared public here because somehow the LLVM compiler does not recognize it from the base class Event
        using Function = std::function<void(Args... args)>;
	...
	public:
        ...
        template <typename U, typename F> Slot(U* parent, F memberFunction) : SignalSlotBase<Args...>(std::bind(memberFunction, parent, std::placeholders::_1)) { }
        ...
        template <typename U, typename F> Slot(U* parent, F memberFunction, Signal<Args...>& signal) : SignalSlotBase<Args...>(std::bind(memberFunction, parent, std::placeholders::_1))
        {
            signal.connect(*this);
        }
...
#define NSLOT(NAME, TYPE, FUNCTION)	\
	nap::Slot<TYPE> NAME = { [&](TYPE inValue)->void { FUNCTION(inValue); } };

// Creates a slot with @NAME and @TYPE without binding it to a function
#define CREATE_SLOT(NAME, TYPE)	\
	nap::Slot<TYPE> NAME;

// Binds the already defined slot with @NAME and @TYPE to @FUNCTION
#define BIND_SLOT(NAME, TYPE, FUNCTION) \
	NAME([&](TYPE inValue)->void { FUNCTION(inValue); } )

// SIGNAL MACRO
#define NSIGNAL(NAME, TYPE) \
	nap::Signal<TYPE> NAME;
